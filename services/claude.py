...
### 5. 才能・特性・使命
各惑星のハウス配置から以下を箇条書きで出力する。
きれいな文章にしない。事実ベースで短く出す。

才能（自然にできること）：〜（惑星・星座・ハウスを根拠に1行）
特性（行動・思考のパターン）：〜（惑星・星座・ハウスを根拠に1行）
使命（社会に出るエネルギー）：〜（惑星・星座・ハウスを根拠に1行）

複数ある場合は複数行出す。

### 6. 今のあなたへ（100字以内）
orb2度以内のアスペクトから一つだけテーマを選ぶ。
そのテーマに直結した具体的な問いで終わる。
漠然とした問い（「あなたは何をしたいですか」等）にしない。
例：「太陽合金星がある人に問う。あなたが美しいと思うものを、今日一つ言語化できるか。」
...
## 出力ルール
- 各セクションに見出しをつける
- セクション1〜4合計1000〜1200字、セクション5は箇条書き、セクション6は100字以内
- 星座名・ハウス番号は最小限に留める"""
